...
class FileUploadUI {
    constructor(dbManager, requiredFiles = {}) {
        ...
        this.requiredFiles = requiredFiles; // { fasta: 'GCF_000146045.fna', vcf: [...], bam: [...] }
    ...
    async checkVersionMismatch(currentVersion) {
        if (!currentVersion || currentVersion === 'None') {
            return { mismatch: false, reason: 'no-versioning' }; // No versioning enabled
        }

        const storedVersion = await this.dbManager.getStoredVersion();

        if (!storedVersion) {
            console.log('No stored version found - files may be from old report');
            return { mismatch: true, reason: 'no-stored-version' };
        }

        if (storedVersion !== currentVersion) {
            console.warn(`Version mismatch! Stored: ${storedVersion}, Current: ${currentVersion}`);
            return { mismatch: true, reason: 'version-changed' };
        }

        console.log(`Version match: ${currentVersion}`);
    ...
    async showModal(currentVersion = null) {
        ...
        // Only skip modal if files exist AND version is confirmed to match
        if (allExist && !versionCheck.mismatch && versionCheck.reason === 'version-match') {
            console.log('All required files already in IndexedDB with correct version');
            return true;
        }

        if (versionCheck.mismatch) {
            if (versionCheck.reason === 'no-stored-version') {
                console.log('Files exist but no version info - prompting re-upload for safety');
            } else if (versionCheck.reason === 'version-changed') {
                console.log('Version mismatch detected - prompting user to re-upload files');
            }
        } else {
    ...
    createModalHTML(missingFiles, versionMismatch = false, reason = null) {
        ...
        const modalHTML = `
            <div id="${this.modalId}" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 100000; justify-content: center; align-items: center;">
                <div style="background: white; border-radius: 8px; padding: 32px; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
                    ...
                    <div id="upload-form" style="display: ${this.hasFileSystemAccess ? 'none' : 'block'};">
                        ...
                        <div class="upload-section" style="margin-bottom: 20px;">
                            ...
                            <input type="file" id="upload-fasta" accept=".fna,.fa,.fasta" style="display: block; margin-bottom: 4px; padding: 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                            <input type="file" id="upload-fai" accept=".fai" style="display: block; padding: 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                        ...
                        <!-- VCF Files -->
                        ${(this.requiredFiles.vcf || []).map((vcfFile, i) => `
                            <div class="upload-section" style="margin-bottom: 20px;">
                                <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #333;">
                                    VCF File ${i + 1}
                                    <span style="color: #e53e3e;">*</span>
                                ...
                                <input type="file" id="upload-vcf-${i}" accept=".vcf,.vcf.gz" data-filename="${vcfFile}" style="display: block; margin-bottom: 4px; padding: 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                                ${vcfFile.endsWith('.gz') ? `
                                    <input type="file" id="upload-tbi-${i}" accept=".tbi" data-filename="${vcfFile}.tbi" style="display: block; padding: 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                                    <p style="margin: 4px 0 0 0; font-size: 12px; color: #999;">
                                        Also upload the .tbi index file
                                    </p>
                                ` : ''}
                            </div>
                        `).join('')}
                        ...
                        ${(this.requiredFiles.bam || []).length > 0 ? `
                            <div class="upload-section" style="margin-bottom: 20px;">
                                ...
                                ${(this.requiredFiles.bam || []).map((bamFile, i) => `
                                    <div style="margin-bottom: 12px;">
                                        <p style="margin: 0 0 8px 0; font-size: 14px; color: #666;">
                                            ${bamFile}
                                        </p>
                                        <input type="file" id="upload-bam-${i}" accept=".bam" data-filename="${bamFile}" style="display: block; margin-bottom: 4px; padding: 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                                        <input type="file" id="upload-bai-${i}" accept=".bai" data-filename="${bamFile}.bai" style="display: block; padding: 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                                    </div>
                                `).join('')}
                            </div>
                        ` : ''}
        ...
        document.getElementById('clear-storage-btn').addEventListener('click', () => this.handleClearStorage());

        // Attach auto-upload listener if available
        ...
        try {
            ...
            for (const input of fileInputs) {
                if (input.files && input.files[0]) {
                    ...
                    const expectedName = input.dataset.filename || this.getExpectedFilename(input.id, file.name);
    ...
    async uploadFiles(filesToUpload) {
        ...
        // Upload files with time estimation
        ...
        // Update cache status display if function is available
    ...
    async handleAutoUpload() {
        ...
        try {
            ...
            const dirHandle = await window.showDirectoryPicker({
                mode: 'read',
                startIn: 'downloads' // Suggest downloads/documents folder
            });

            console.log('Directory selected:', dirHandle.name);

            // Show progress bar immediately after folder selection
            ...
            console.log('Files in selected folder:', allFilesInDir);

            // Progress: 20% - folder scanned
            if (progressBar) progressBar.style.width = '20%';
            if (progressText) progressText.textContent = '20%';
            if (statusText) statusText.textContent = `🔍 Found ${allFilesInDir.length} files, checking required files...`;

            // Build list of all required filenames
            ...
            console.log('Looking for these files:', requiredFilenames);
            ...
            for (let i = 0; i < requiredFilenames.length; i++) {
                ...
                try {
                    ...
                    // Update progress as we find each file (20% -> 40%)
            ...
            if (missingFiles.length > 0) {
                throw new Error(`Missing ${missingFiles.length} file(s) in selected folder:\n\n${missingFiles.join('\n')}\n\nPlease select the correct folder or use manual upload.`);
            }
            ...
            console.error('Auto-upload error details:', {
                name: error.name,
            ...
            if (error.name === 'AbortError') {
                ...
                if (statusText) statusText.textContent = 'Folder selection cancelled. You can try again or use manual upload below.';
                return;
            }
    ...
    autoFillFileInputs(foundFiles) {
        for (const { file, name } of foundFiles) {
            ...
            if (input) {
                ...
                console.log(`Auto-filled input for: ${name}`);
            }
        }
    }
    ...
    findInputForFilename(filename) {
        // Try to find by data-filename attribute first
        let input = document.querySelector(`input[type="file"][data-filename="${filename}"]`);
        if (input) return input;

        // Check specific input IDs based on filename patterns
    ...
    async handleClearStorage() {
        if (!confirm('Are you sure you want to delete the entire database? You will need to re-upload all files.')) {
            return;
        }
